...
<div class="display accounts subproject">
	<div class="floaticon">
		{if $print_page ne 'y'}
			{* =-=- CUSTOM BEGIN: icons -=-= *}
			...
			{if $gContent->hasUpdatePermission()}
				<a title="{tr}Edit this {$gContent->getContentTypeName()|strtolower}{/tr}" href="{$smarty.const.ACCOUNTS_PKG_URL}edit_subproject.php?subproject_id={$gContent->mInfo.subproject_id}">{biticon ipackage="icons" iname="accessories-text-editor" iexplain="Edit `$gContent->mType.content_name`"}</a>
			{/if}
			{if $gContent->hasExpungePermission()}
				<a title="{tr}Remove this {$gContent->getContentTypeName()|strtolower}{/tr}" href="{$smarty.const.ACCOUNTS_PKG_URL}remove_subproject.php?subproject_id={$gContent->mInfo.subproject_id}">{biticon ipackage="icons" iname="edit-delete" iexplain="Remove Subproject"}</a>
			{/if}
		{/if}<!-- end print_page -->
	</div><!-- end .floaticon -->
