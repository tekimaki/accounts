...
<div class="display accounts project">
	<div class="floaticon">
		{if $print_page ne 'y'}
			{* =-=- CUSTOM BEGIN: icons -=-= *}
			...
			{if $gContent->hasUpdatePermission()}
				<a title="{tr}Edit this {$gContent->getContentTypeName()|strtolower}{/tr}" href="{$smarty.const.ACCOUNTS_PKG_URL}edit_project.php?project_id={$gContent->mInfo.project_id}">{biticon ipackage="icons" iname="accessories-text-editor" iexplain="Edit `$gContent->mType.content_name`"}</a>
			{/if}
			{if $gContent->hasExpungePermission()}
				<a title="{tr}Remove this {$gContent->getContentTypeName()|strtolower}{/tr}" href="{$smarty.const.ACCOUNTS_PKG_URL}remove_project.php?project_id={$gContent->mInfo.project_id}">{biticon ipackage="icons" iname="edit-delete" iexplain="Remove Project"}</a>
			{/if}
		{/if}<!-- end print_page -->
	</div><!-- end .floaticon -->
